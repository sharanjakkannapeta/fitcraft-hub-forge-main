    ...
    <header className="sticky top-0 z-50 border-b border-border bg-background/85 backdrop-blur-xl">
      <div className="container mx-auto flex h-16 items-center justify-between px-4">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          {links.map((l) => (
            <Link
              key={l.to}
              to={l.to}
              className="text-sm font-medium uppercase tracking-wide text-muted-foreground transition-smooth hover:text-foreground"
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={() => nav({ to: "/shop" })}
            className="hidden h-9 w-9 items-center justify-center rounded-full text-foreground transition-smooth hover:bg-secondary md:inline-flex"
            aria-label="Search"
          >
          ...
          {user ? (
            <>
              ...
              {isAdmin && (
                <Link
                  to="/admin"
                  className="hidden rounded-md border border-border px-3 py-1 text-xs font-medium uppercase tracking-wide transition-smooth hover:bg-secondary md:inline-block"
                >
                  Admin
                </Link>
              )}
            ...
            <Link
              to="/login"
              className="rounded-md bg-foreground px-4 py-2 text-xs font-semibold uppercase tracking-wider text-primary-foreground transition-smooth hover:bg-foreground/85"
            >
              Sign in
            </Link>
          )}
